...
def has_gain():
    if (SWING_PRICE > 0 and in_position):
        ...
        if (percent >= SWING_MARGIN):
            msg = "Has gain. \n*** [{} -> {} = {}%] ***".format(SWING_PRICE,
                                                                actual_price, float("{:.2f}".format(percent)))
            print(msg)
            messenger.send(msg)
            sell()
...
def make_historical():
    ...
    historical = client.get_historical_klines(TRADE_SYMBOL, Client.KLINE_INTERVAL_1MINUTE, "01 Feb, 2023", DATE_NOW)
    for h in historical:
